...
let lookup_class : string -> class_env -> class_decl ea_result  = fun c_name c_env ->
  ...
  | Some class_decl -> return @@ class_decl
    
let new_env : string list -> env ea_result  = fun fs -> 
  let rec new_env_helper = fun fs ->
    match fs with
    | [] -> empty_env()
    | h::tl -> let r = (RefVal (Store.new_ref g_store (NumVal 0))) in
      new_env_helper tl >>+
      extend_env h r
  in new_env_helper (List.rev fs)
  
  (* (fun ids vs ->
  List.fold_left2 (fun ac id v -> ac >>+ extend_env id v) (empty_env()) ids vs)
  fs (List.init (List.length fs) (fun _ -> (RefVal (Store.new_ref g_store (NumVal 0))))) *)
...
let lookup_method : string -> string -> class_env ->
  ...
  match List.assoc_opt c_name c_env with
  | Some (_super,_fields,methods) -> List.assoc_opt m_name methods
  | None -> None
